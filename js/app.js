...
async function pullFromSheets(silent = false, force = false) {
  // Only small data syncs via Sheets: clients, recentDocs, counter, markup.
  // Materials and LaborRates are local-only per device — never fetched here.
  ...
  // Counter: NEVER move backward, even with force=true. The whole point of
  // syncing this is to prevent duplicate invoice numbers, so we always keep
  // whichever counter is furthest along. If Sheets is behind local (e.g. a
  // push hasn't landed yet), push local up instead of pulling the old value down.
  if (res.counters) {
    const remoteVal = counterValue(res.counters);
    const localVal = counterValue(STATE.counters);
    if (remoteVal > localVal) {
      STATE.counters = res.counters; saveLocal('counters'); changed = true;
    } else if (localVal > remoteVal) {
      gasRequest('setCounter', STATE.counters); // heal stale Sheets value
    }
  }
  if (res.markup) {
  ...
  if (!silent) {
    showToast(`Sync ✓ — ${STATE.clients.length} clientes · próximo #${docNumString()}`);
  }
...
// Force-push this device's Clients + RecentDocs + Counter to Sheets,
// overwriting whatever is there. Use this to make one device's data
// the source of truth (e.g. after fixing a duplicate client).
// Materials and LaborRates are never touched here — they stay local-only.
async function writeAllToSheets() {
  if (!STATE.gasUrl) { showToast('No conectado a Sheets', true); return; }

  showToast(`Subiendo ${STATE.clients.length} clientes…`);

  // Clients — clear then write in batches of 25
  await gasRequest('clearSheet', { sheet: 'Clients' });
  for (let i = 0; i < STATE.clients.length; i += 25) {
    const batch = STATE.clients.slice(i, i + 25);
    const res = await gasRequest('appendBatch', { sheet: 'Clients', rows: batch });
    if (!res || !res.ok) {
      showToast(`Error al subir clientes (batch ${i/25 + 1})`, true);
      return;
    }
  }

  // RecentDocs — clear then write in batches of 25
  await gasRequest('clearSheet', { sheet: 'Docs' });
  for (let i = 0; i < STATE.recentDocs.length; i += 25) {
    const batch = STATE.recentDocs.slice(i, i + 25);
    const res = await gasRequest('appendBatch', { sheet: 'Docs', rows: batch });
    if (!res || !res.ok) {
      showToast(`Error al subir documentos (batch ${i/25 + 1})`, true);
      return;
    }
  }
...
// Turns {year, quarter, seq} into a single comparable number so we can
// tell which of two counters is chronologically further along — this is
// what lets sync guarantee the invoice counter only ever moves forward.
function counterValue(c) {
  if (!c) return -1;
  const yr = parseInt(c.year) || 0;
  const q = parseInt(c.quarter) || 0;
  const seq = parseInt(c.seq) || 0;
  return yr * 1000 + q * 100 + seq;
}
...
function saveItemToCatalog(rowId) {
  ...
  if (type === 'labor') {
    ...
    saveLocal('laborRates'); // local-only, no Sheets sync
    renderLaborList();
    ...
    saveLocal('materials'); // local-only, no Sheets sync
    renderMaterialList();
...
async function importMaterials(event) {
  ...
  // Materials are local-only — save locally and stop. No Sheets sync.
  saveLocal('materials');
  renderAllLists();
...
async function importLaborRates(event) {
  ...
  // Labor rates are local-only — save locally and stop. No Sheets sync.
...
// ── SETTINGS ─────────────────────────────────────────────────
function togglePaid() {
  const cb = document.getElementById('fac-paid');
  const indicator = document.getElementById('paid-indicator');
  ...
  if (cb.checked) {
    ...
    toggle.style.background = '#eafaf1';
    label.textContent = 'PAGADA ✓';
...
function setCounter() {
  ...
  // Preserve current year/quarter, only update seq
  const { yr, q } = getCurrentYQ();
  STATE.counters = { year: yr, quarter: q, seq: val };
  saveLocal('counters');
  // Push to Sheets immediately so all devices see the updated counter
  if (STATE.gasUrl) gasRequest('setCounter', STATE.counters);
  updateDocNumber();
  input.value = '';
